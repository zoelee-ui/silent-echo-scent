        ...
        onclone: (clonedDoc) => {
          ...
          if (card) {
            ...
            if (bgOverlay) {
              ...
              bgOverlay.style.background = `radial-gradient(circle at center, ${res.hex} 0%, transparent 80%)`;
            }
          }
        }
      });
  ...
  return (
    <main className="min-h-screen bg-[#FDFDFD] flex items-center justify-center p-4 md:p-12 text-black select-none relative overflow-hidden font-sans" onMouseMove={handleMouseMove}>
      ...
      <nav className="fixed top-0 left-0 w-full z-[110] px-6 md:px-12 py-6 md:py-8 flex justify-between items-center bg-white/5 backdrop-blur-md border-b border-black/[0.03]">
        <div className="text-[9px] md:text-[11px] tracking-[0.8em] font-light opacity-60 uppercase">SILENT ECHO</div>
        ...
        <div className="h-full flex flex-col items-center justify-center gap-10 md:gap-14">
          ...
          {!vipOpen ? (
            ...
            <div className="flex flex-col items-center animate-[visionFocus_0.6s_forwards] max-w-[320px] text-center px-6">
              {vipStage === 'mission' ? (
                <>
                  <p className="text-[9px] md:text-[10px] tracking-[0.3em] leading-[2.2] text-black/40 mb-10 uppercase">SHARE THIS PAGE TO IG STORY AND TAG @SILENT_ECHO<br/>DM THE SCREENSHOT TO GET YOUR ACCESS CODE</p>
      ...
      <div className={`fixed inset-0 z-[120] flex items-center justify-center p-4 md:p-8 transition-all duration-1000 ease-in-out ${storyOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="relative max-w-2xl w-full bg-[#FDFDFD] p-10 md:p-24 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.03)] border border-black/[0.02] overflow-y-auto max-h-[85vh] custom-scrollbar text-center">
          ...
          <div className="space-y-16 text-left max-w-md mx-auto">
            <section><p className="text-[8px] tracking-[0.5em] opacity-20 mb-4 uppercase pl-[0.5em]">MISSION</p><p className="text-[13px] md:text-[14px] leading-[2.4] font-light opacity-60 tracking-[0.15em]">在這個資訊過載的時代，氣味不應成為另一種選擇壓力。Silent Echo 致力於消除挑選香水時的焦慮，我們不談論成分，只談論你的頻率。</p></section>
            <section><p className="text-[8px] tracking-[0.5em] opacity-20 mb-4 uppercase pl-[0.5em]">RESONANCE</p><p className="text-[13px] md:text-[14px] leading-[2.4] font-light opacity-60 tracking-[0.15em]">如果您曾迷失於百貨公司的香氛專櫃，這裡將是您的歸宿。我們透過光影、節奏與留白，幫助您媒合出那支能與您靈魂共振的「回聲」。</p></section>
          </div>
        </div>
      </div>

      {/* VIP DASHBOARD */}
      {step === 'vip_dashboard' && (isVisible) && (
        <div className={`max-w-5xl w-full h-[80vh] flex flex-col px-4 md:px-10 ${fadeClass}`}>
          <div className="flex justify-between items-end mb-12 md:mb-16 border-b border-black/[0.05] pb-8">
            <div className="text-left"><p className="text-[8px] tracking-[1em] opacity-20 mb-2 uppercase pl-[1em]">EXCLUSIVE ACCESS</p><h1 className="text-xl md:text-2xl font-light tracking-[0.4em] uppercase">PRIVATE ARCHIVE</h1></div>
            <button onClick={() => { setIsVisible(false); setTimeout(() => { setStep('login'); setIsVisible(true); }, 800); }} className="flex items-center gap-2 opacity-20 hover:opacity-100 text-[8px] tracking-[0.4em] transition-all uppercase"><ArrowLeft size={14} strokeWidth={1} /> BACK HOME</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 lg:gap-x-20 gap-y-16 overflow-y-auto pr-4 custom-scrollbar flex-grow pb-10">
            {Object.values(PERFUME_MATCHES).map((item: any, idx) => (
              <div key={idx} className="group space-y-8 border-l border-black/[0.03] pl-8">
                <div className="flex items-center justify-between"><span className="text-[7px] tracking-widest opacity-20 uppercase">0{idx + 1} / COLLECTION</span><div className="w-2 h-2 rounded-full" style={{ backgroundColor: item.hex }} /></div>
                <div className="space-y-2"><h3 className="text-[10px] tracking-[0.5em] opacity-30 uppercase">{item.brand}</h3><h2 className="text-lg md:text-xl font-light tracking-wider uppercase">{item.title}</h2></div>
          ...
          <div className="mt-8 p-6 md:p-8 border border-dashed border-black/10 bg-black/[0.01] flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="text-center md:text-left"><p className="text-[8px] tracking-[1em] opacity-20 mb-3 uppercase pl-[1em]">REWARD</p><h2 className="text-sm tracking-[0.3em] font-light uppercase">VIP EXCLUSIVE: 15% OFF DISCOUNT CODE</h2></div>
      ...
      {step === 'login' && (
        <div className={`w-full flex flex-col items-center justify-center gap-20 md:gap-28 ${fadeClass}`}>
          <span className="text-[10px] md:text-[12px] tracking-[1.5em] md:pl-[1.5em] text-black/40 uppercase font-light text-center">FIND YOUR OWN SCENT</span>
          <button onClick={() => { setIsVisible(false); setTimeout(() => { setStep('quiz'); setIsVisible(true); }, 900); }} className="group relative px-14 py-4 md:px-20 md:py-6 overflow-hidden border border-black/10 hover:border-black transition-colors duration-500 uppercase">
            <span className="relative z-10 text-[10px] md:text-[12px] tracking-[0.6em] group-hover:text-white transition-colors duration-500 pl-[0.6em]">START</span>
          ...
          <div className="flex flex-col gap-10 md:gap-14">
            {questions[qIndex].options.map((opt, i) => (
              <button key={i} onClick={() => handleNextQuiz(opt.v)} className="group text-[11px] md:text-[13px] tracking-[0.4em] text-black/40 hover:text-black transition-all duration-500 flex flex-col items-center uppercase">
                <span className="mb-2 group-hover:tracking-[0.8em] group-hover:text-black transition-all duration-700">{opt.t}</span>
                <div className="w-0 h-[0.5px] bg-black/40 group-hover:w-28 md:group-hover:w-40 group-hover:bg-black transition-all duration-700"></div>
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 'particle' && (
        <div className={`fixed inset-0 flex items-center justify-center transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="absolute inset-0 overflow-hidden">
            ...
            {[...Array(3)].map((_, i) => (<div key={i} className="absolute inset-0 opacity-40 animate-[nebulaFlow_15s_infinite]" style={{ background: `radial-gradient(circle at ${30+i*20}% ${40+i*10}%, white 0%, transparent 70%)`, animationDelay: `${i*-5}s` }} />))}
          </div>
      ...
      {step === 'result' && (
        <div className="w-full flex flex-col items-center justify-center min-h-[80vh] py-10">
          <div ref={resultRef} data-result-card="true" className={`w-full max-w-[420px] md:max-w-[460px] bg-[#FDFDFD] text-black p-12 md:p-16 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.05)] border border-black/[0.02] relative overflow-hidden flex flex-col items-center ${isCapturing ? '!filter-none !opacity-100' : ''}`} style={{ animation: isCapturing ? 'none' : 'visionFocus 3s forwards' }}>
            <div data-bg-overlay="true" className="absolute inset-0 z-0 pointer-events-none transition-opacity duration-1000" style={{ background: `radial-gradient(circle at center, ${res.hex} 0%, transparent 80%)`, opacity: isCapturing ? 0.35 : 0.15 }} >
                {!isCapturing && <div className="absolute inset-0 animate-[lowSaturatePulse_8s_infinite] transition-opacity duration-[3000ms]" style={{ background: `radial-gradient(circle at center, ${res.hex} 0%, transparent 70%)` }} />}
            </div>
            <div className="relative z-10 w-full text-center flex flex-col items-center">
              <div className="flex justify-between w-full text-[7px] md:text-[8px] tracking-[0.4em] opacity-30 mb-14 md:mb-20 uppercase"><span>PANTONE® {res.pantone}</span><span>{res.tag}</span></div>
              <p className="text-[10px] md:text-[11px] tracking-[0.4em] text-black/30 mb-2 uppercase pl-[0.4em]">{res.brand}</p>
              <h1 className="text-3xl md:text-4xl font-light mb-8 md:mb-12 tracking-[0.1em] uppercase leading-tight">{res.title}</h1>
              <p className="text-[11px] md:text-[12px] text-black/50 italic mb-16 md:mb-20 leading-relaxed px-4">{res.description}</p>
              <div className="w-full flex flex-col items-center">
                <div className="w-full h-[0.5px] bg-black/5 mb-14 md:mb-16" />
                <div className="grid grid-cols-1 gap-12 md:gap-16 py-8 md:py-10 mb-16">
                   {Object.entries(res.notes).map(([key, note]: any) => (
                     <div key={key} className="flex flex-col items-center uppercase">
                       <span className="text-[6px] md:text-[7px] tracking-[0.8em] text-black/10 mb-1">{key} NOTE</span>
                       <span className="text-[11px] md:text-[13px] tracking-[0.2em] text-black/70 px-4 leading-relaxed">{note}</span>
                     </div>
                   ))}
                </div>
                <div className="w-full h-[0.5px] bg-black/5 mb-16 md:mb-20" />
              </div>
              <div className="flex justify-center gap-12 md:gap-16 mt-4">
                <button onClick={saveResultCard} className="group flex flex-col items-center gap-2 opacity-20 hover:opacity-100 transition-all duration-700 ease-in-out">
                  <Download size={18} strokeWidth={1} /><span className="text-[7px] md:text-[8px] tracking-widest pl-[0.2em] uppercase">SAVE MATCH</span>
                </button>
                <button onClick={() => window.location.reload()} className="group flex flex-col items-center gap-2 opacity-20 hover:opacity-100 transition-all duration-700 ease-in-out">
                  <RotateCcw size={18} strokeWidth={1} className="group-hover:rotate-45 transition-transform duration-700" /><span className="text-[7px] md:text-[8px] tracking-widest pl-[0.2em] uppercase">RETRY</span>
                </button>
              </div>
            </div>
          </div>
