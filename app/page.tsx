    ...
    <main className="min-h-screen bg-[#FDFDFD] flex items-center justify-center p-6 text-black select-none relative overflow-hidden font-sans" onMouseMove={handleMouseMove}>
        ...
        <div className="h-full flex flex-col items-center justify-center gap-12">
          ...
          {!vipOpen ? (
            ...
            <div className="flex flex-col items-center max-w-[280px] text-center">
              {vipStage === 'mission' ? (
                <>
                  <p className="text-[9px] tracking-[0.3em] leading-[2.2] text-black/40 mb-10 uppercase">SHARE THIS PAGE TO IG STORY AND TAG @SILENT_ECHO<br/>DM THE SCREENSHOT TO GET ACCESS CODE</p>
                  <button onClick={() => setVipStage('input')} className="text-[8px] tracking-[0.6em] border border-black/10 px-6 py-2 hover:bg-black hover:text-white uppercase">I HAVE A CODE</button>
                </>
              ) : (
      ...
      <div className={`fixed inset-0 z-[120] flex items-center justify-center p-8 transition-all duration-1000 ease-in-out ${storyOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="relative max-w-xl w-full bg-[#FDFDFD] p-12 lg:p-20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.03)] border border-black/[0.02] overflow-y-auto max-h-[80vh] custom-scrollbar">
          ...
          <div className="text-center">
            ...
            <h2 className="text-[10px] tracking-[1.2em] opacity-30 mb-20 uppercase pl-[1.2em] reveal-item">THE PHILOSOPHY</h2>
            <div className="space-y-16 text-left">
              <section className="reveal-item reveal-delay-1">
                <p className="text-[8px] tracking-[0.5em] opacity-20 mb-4 uppercase pl-[0.5em]">MISSION</p>
                <p className="text-[13px] leading-[2.4] font-light opacity-60 tracking-[0.15em]">在這個資訊過載的時代，氣味不應成為另一種選擇壓力。Silent Echo 致力於消除挑選香水時的焦慮，我們不談論成分，只談論你的頻率。</p>
              </section>
              <section className="reveal-item reveal-delay-2">
                <p className="text-[8px] tracking-[0.5em] opacity-20 mb-4 uppercase pl-[0.5em]">RESONANCE</p>
                <p className="text-[13px] leading-[2.4] font-light opacity-60 tracking-[0.15em]">如果你曾迷失於百貨公司的香氛專櫃，或是無法描述內心渴望的氣息，這裡將是你的歸宿。我們透過光影、節奏與留白，幫助你媒合出那支能與你靈魂共振的「回聲」。</p>
              </section>
              <section className="reveal-item reveal-delay-3">
                ...
                <p className="text-[13px] leading-[2.4] font-light opacity-60 tracking-[0.15em]">這是一場關於自我探尋的數位儀式。當你準備好關掉噪音，氣味就會浮現。</p>
              </section>
            </div>
          </div>
        </div>
      ...
      {step === 'login' && (
        <div className={`w-full flex flex-col items-center justify-center gap-20 ${fadeClass}`}>
          <span className="text-[10px] tracking-[1.5em] pl-[1.5em] text-black/40 uppercase font-light">FIND YOUR OWN SCENT</span>
          <button onClick={() => { setIsVisible(false); setTimeout(() => { setStep('quiz'); setIsVisible(true); }, 900); }} className="group relative px-14 py-4 overflow-hidden border border-black/10 hover:border-black transition-colors duration-500 uppercase">
            <span className="relative z-10 text-[10px] tracking-[0.6em] group-hover:text-white transition-colors duration-500 pl-[0.6em]">START</span>
          ...
          <div className="flex flex-col gap-10">
            {questions[qIndex].options.map((opt, i) => (
              <button key={i} onClick={() => handleNextQuiz(opt.v)} className="group text-[11px] tracking-[0.4em] text-black/40 hover:text-black transition-all duration-500 flex flex-col items-center uppercase">
                <span className="mb-2 group-hover:tracking-[0.8em] transition-all duration-700">{opt.t}</span>
                <div className="w-0 h-[0.5px] bg-black/40 group-hover:w-24 group-hover:bg-black transition-all duration-700" />
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 'particle' && (
        <div className={`fixed inset-0 flex items-center justify-center transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
          <div className="absolute z-20 text-[10px] tracking-[1.2em] text-black/20 uppercase" style={{ opacity: Math.max(0, 1 - progress / 50) }}>DRAW A CIRCLE IN VOID</div>
      ...
      {step === 'result' && (
        <div ref={resultRef} className={`w-full max-w-[420px] bg-[#FDFDFD] text-black p-16 border border-black/[0.02] relative overflow-hidden`} style={{ animation: 'visionFocus 3s forwards' }}>
          <div className="relative z-10 text-center">
            ...
            <button onClick={saveResultCard} className="opacity-20 hover:opacity-100 transition-all uppercase text-[7px] tracking-widest">SAVE MATCH</button>
          </div>
        </div>
      )}
    </main>
